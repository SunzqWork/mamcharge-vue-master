      ...
      <el-table :height="tableHeight" border class="zl-table" :data="tableData" @selection-change="handleSelectionChange">
        ...
        <el-table-column align="center" label="操作" width="250">
          <template slot-scope="scope">
            ...
            <div v-if="scope.row.edit">
              <span class="icon-theme" type="success" @click="save(scope.row)">确定</span>
              <span class="icon-theme" type="danger" @click="deleteSingle(scope.row)">删除</span>
              <span class="icon-theme" @click="scope.row.edit = !scope.row.edit" type="info">取消</span>
            </div>
            <div v-else>
              <span class="icon-theme" @click="edits(scope.row)" type="primary">编辑</span>
            </div>
            </div>
          </template>
        </el-table-column>
    ...
    <el-dialog title="新增字段" :visible.sync="dialogVisible" width="850px">
      <el-form style="overflow:hidden;" :rules="rules" label-width="135px" ref="form" :model="form">
        ...
        <el-form-item style="float:left;width: 50%;height: 36px;" label="字段类型" prop="colType">
          <el-select style="width: 100%;" v-model="form.colType" filterable placeholder="请选择">
            <el-option v-for="item in sqlType" :key="item" :label="item" :value="item"></el-option>
          </el-select>
        </el-form-item>
...
<script>
import { extOrg } from "@/api/help"
import { getOrgCustomUser, getOrgCustomPut, deleteOrgCustomPut } from "@/api/custom";
// 字段类型需要写死
const sqlType = [
  "tinyint",
  "smallint",
...
export default {
  ...
  methods: {
    ...
    afterDelete(con) {
      ...
      return getOrgCustomPut(con).then(s => {
      });
    },
    // 单个删除
    async deleteSingle(row) {
      await this.afterDelete([row]);
      deleteOrgCustomPut([row.id])
        .then(
          req => {
            req.success
              ? this.$message({
